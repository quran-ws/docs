...
- **`ar` و`en` متقابلان ملفًا بملف.** الصفحة الموجودة في لغة واحدة نقص معروف، لا بنية مختلفة. العربية أصل لصفحات النص القرآني والمصطلحات، والإنجليزية أصل لصفحات الهندسة.
- **كل صفحة تذكر حالها في الـ`frontmatter`**: `draft` أو `proposed` أو `adopted`. ولا يلزم مشاريعنا إلا ما وُسم `adopted`.
- **المفهوم له مدخل، والفرد له صف.** `concepts/` لما يحتاج إلى تعريف، و`registries/` للمجموعات المغلقة التي لا يقول أفرادها شيئًا غير أسمائهم ومواضعهم في المجموعة: 10 قراءات، و114 سورة. ومدخل المفهوم يسمي سجله، و`tools/check_registries.py` يفحص الصفوف.
...
- **`surveys/` نتائج، و`examples/` نماذج مفحوصة.** الفحص لا يغيّر المعيار، وإنما يصير حالةً وراء اقتراح. والنماذج يفحصها البناء، فما فيها هو ما يقوله المعيار اليوم.
