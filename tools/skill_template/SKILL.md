...
>
> Snapshot `{{SNAPSHOT}}` of `{{REPO}}`. The dictionary keeps moving, so before an
...
| `references/dictionary.md` | the {{ENTRIES}} entries as prose, with a lookup table; `lookup.py` is faster |
