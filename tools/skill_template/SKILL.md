...
This skill carries the standard, its dictionary of {{ENTRIES}} concepts and
{{SPELLINGS}} spellings, the registries of the closed sets, and scripts that
answer a naming question without guessing.

> Status: {{ADOPTED}} entries adopted, {{DRAFT}} still draft. A draft entry is a
> proposal, not a ruling — say so when you rely on one.
>
> Built from `{{REPO}}`@`{{COMMIT}}`. The dictionary keeps moving, so before an
...
Registries (closed sets, one row per member): {{REGISTRIES}}.
...
- Run `scripts/update_check.py` when you pick the skill up again. When it says
  behind, refresh before ruling on a name: clone `{{REPO}}`, run
...
| `references/dictionary.md` | the {{ENTRIES}} entries, generated from the same source |
