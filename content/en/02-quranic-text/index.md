...
description: Quranic text is transmitted source data, not content we edit. Practical rules for storage, tokenization, display and testing.
status: draft
sidebar:
  order: 1
---

The Quran is the word of God, and He undertook its preservation: ﴿إِنَّا نَحْنُ
نَزَّلْنَا ٱلذِّكْرَ وَإِنَّا لَهُۥ لَحَٰفِظُونَ﴾ [al-Hijr: 9]. Honouring what He
made sacred is itself an act of piety: ﴿وَمَن يُعَظِّمْ شَعَٰٓئِرَ ٱللَّهِ
فَإِنَّهَا مِن تَقْوَى ٱلْقُلُوبِ﴾ [al-Hajj: 32]. A reader who opens your app is
...
table is at the end. The figures and examples come from an audit of a published
Uthmani text, run for this guide.

## 1. The source text

- Quranic text is transmitted, not authored. You are not its editor.
- Identify a text before you import it: edition, riwayah, ayah numbering system,
  version, and a hash of the file.
...
- Use one declared Unicode encoding end to end: source file, database, API,
  frontend.
- Visually identical is not digitally identical. Compare code points, not shapes.
...
- Never normalize the source to `NFC`, `NFD`, `NFKC` or `NFKD`. Running `NFC` over a
  full mushaf changed **5,771 ayahs out of 6,236**, in two different ways:
...
- `string length` is not the number of visible letters. The audited text is
  **1,360,018 bytes**, **721,236 code points**, and fewer visible letters than
  either.
- Build a whitelist of the characters your text actually uses and fail the build on
...
- Edition marks are not part of an ayah: end of ayah ۝ (`U+06DD`), rub' ۞
...
## 4. Tokenization, identifiers and offsets

- Give surahs, ayahs and words stable identifiers. The text itself is never the key.
- Define your tokenization explicitly. A programmatic word does not always match a
  word in the mushaf:
...
- Splitting the audited text on whitespace yields **82,456** tokens — not the word
  count of the mushaf, and 199 of them are rub' marks.
...
## 5. Riwayat, editions and numbering systems

- Never mix data from different riwayat or editions, however identical the text
  looks.
...
- The basmalah is its own field: whether it counts as an ayah depends on the
  numbering system, and Surah at-Tawbah has none.
- Test counts against the edition and numbering system you actually ship, not
  against remembered constants. 6,236 is the output of one numbering system, not a
  universal fact. Reference:
...
- The search field (`search_key`) is derived: strip vowels and tatweel there, unify
  hamzah, alef, yaa and taa forms there. Never show it to the reader, never store it
...
- When you must cut the string itself — `og:description`, a notification — cut on
  ...
  its marks.
- A word boundary is not a legitimate stopping place. Stopping at (107:4) without
  (107:5) reverses the meaning.
...
| No normalization | A `lint` rule banning `normalize` in the text path |
| No foreign characters | Character set checked against the whitelist generated from the source |
