...
قواعد الأقسام من 4 إلى 8 كلها قواعد آلية، فتُشتق الـCanonical Code Spelling من
الاسم العربي بالحركات بدالة واحدة بدل تركها للاجتهاد.
...
- الدالة **غير قابلة للعكس** بقصد؛ فالحروف المفخمة ونظائرها تلتقي في حرف
  لاتيني واحد (`ص` و`س` كلاهما `s`)، لأن الهدف ثبات المعرف لا الدقة الصوتية.
...
اسم الحرف صوت قبل كل شيء: `نُون` هو `/nuːn/`، ولا معنى للاسم خارج هذا الصوت،
فاشتقاقه حرفًا حرفًا يضيع الشيء الوحيد الذي يحمله. لذلك تكتب أسماء الحروف
بصورتها المنطوقة المستقرة:
...
وفي هذا فائدة ثانية: `nun` و`sin` كلمتان إنجليزيتان لهما معنى آخر بعيد، فلا
يحسن أن يشحن بهما معرف في الكود.

**والقاعدة خاصة باسم الحرف وحده.** أما بقية المصطلحات فتشتق كما في الأقسام
4 إلى 8:
...
للتسمية 4 وظائف مطالبها متعارضة: معرف ثابت في الكود، واسم مألوف للقارئ، ونقل
دقيق للنطق، ومدخل للبحث. ولا يؤديها اسم واحد، فيفرد لكل وظيفة حقلها:

```yaml
names:
  code:            nun_sakinah          # المعرف
  display:         Noon Sakinah         # المكتوب للقارئ
  transliteration: nūn sākinah          # النقل الدقيق
  arabic:
...
- **`code`** معرف يستعمل في الكود والـAPI وقواعد البيانات. يشتق بقواعد الأقسام
  4 إلى 8 **بلا استثناء**، والمهم فيه أن يبقى ثابتًا حتى لو لم يكن مألوفًا.
- **`display`** ما يقرؤه الناس في التوثيق والواجهات. يؤخذ من الصورة **الأكثر استعمالًا**
  في الكتابة الإنجليزية، ويسجل في `display_evidence` ما يثبت ذلك، فهو اختيار
  مبني على قياس وليس على الذوق.
- **`transliteration`** النقل الدقيق (ALA-LC أو DIN 31635) لمن احتاجه، وهو اختياري.
- **`arabic.vocalized`** الاسم العربي بالحركات باعتباره أصل الاشتقاق.
- **`alternative_spellings`** كل تهجئة معروفة. يولد منها فهرس يحول أي تهجئة
  إلى مفهومها، في البحث وفي مدخلات الأدوات.

## لماذا يختلف `code` عن `display`؟

الاختلاف مقصود. القياس يبين أن `Noon Sakinah` تستعمل في الكتابة الإنجليزية
4 أضعاف `Nun Sakinah`، وأن `Meem Sakinah` تستعمل 5 أضعاف `Mim Sakinah`.
فالصورة المألوفة مهمة، ومكانها المناسب حيث يقرأ الناس وليس في عمود قاعدة بيانات.

ولو وضعنا الصورة المألوفة في `code` لسقطت قاعدة الاشتقاق. فالتضعيف في
الاستعمال الجاري **يتبع الكلمة لا القاعدة**: يضعف في الكلمات المألوفة ويترك
في غيرها. في ملف واحد تجد `sagheer` و`haqiqi` وفي كليهما ياء مدية واحدة،
وتجد `sughra` و`sagheer` وهما من أصل واحد. والاستعمال الذي لا يطرد لا يصلح أن يكون قاعدة.

فيبقى `code` مشتقًا، وتأخذ الصورة المألوفة مكانها في `display` وفي فهرس المرادفات.
...
بينهما. وأسماء يونيكود ثابتة لا تصحح ولو ثبت خطؤها.

فيسجل اسم يونيكود جهةً من جهات التسمية في `names.unicode`، ويبقى الاسم
البرمجي مشتقًا.

## جهات التسمية الأخرى

للعلامة في المصحف أسماء بجهات مختلفة، ولكل جهة حقلها في `names`:

- `dabt` الاسم الاصطلاحي في علم الضبط.
- `by_shape` الاسم بحسب صورة الرسم.
- `mushaf_introduction` الاسم كما ورد في مقدمة المصحف نفسه؛ وخلوه خبر عن
...
- `status` حاله: `draft` ثم `proposed` ثم `adopted`، ولا يوسم بـ`adopted` أي
  مدخل ليس له مصدر.
- `arabic` لازم لكل مدخل `origin: quranic`، ولا يلزم في المصطلح المستعار إلا
  إذا كان له اسم عربي مستقر، حتى لا يولّد المعيار مصطلحات عربية جديدة دون قصد.
