...
لكل مفهوم عدة أسماء، ولكل اسم حقله:
...
| الحقل | ما هو | كيف يحدد |
| --- | --- | --- |
| `code` | المعرف في الكود والـAPI وقواعد البيانات | يشتق بقواعد الأقسام 4–8 بلا استثناء |
| `display` | المكتوب في التوثيق والواجهات | الصورة الأكثر استعمالًا في الكتابة الإنجليزية، ودليلها في `display_evidence` |
| `transliteration` | النقل الدقيق (ALA-LC أو DIN 31635) | اختياري |
| `arabic.vocalized` | الاسم العربي بالحركات | أصل اشتقاق `code` |
| `dabt` | الاسم الاصطلاحي في علم الضبط | من مصدره |
| `by_shape` | الاسم بحسب صورة الرسم | من مصدره |
| `mushaf_introduction` | الاسم في مقدمة المصحف | `null` إذا لم تسمه المقدمة |
| `unicode` | اسم المحرف في يونيكود | من قاعدة يونيكود |
| `alternative_spellings` | كل تهجئة معروفة | يولد منها فهرس المرادفات |

**قواعد ثابتة:**

- `code` و`display` قد يختلفان، وهذا مقصود. `code` يشتق، و`display` يقاس.
- اسم يونيكود يسجل في `names.unicode` ولا يستعمل معرفًا في الكود، لأنه يصف
  صورة المحرف لا وظيفته، وقد يخدم المحرف الواحد علامتين.
- خلو `mushaf_introduction` خبر عن المصدر، وليس نقصًا في المدخل.
