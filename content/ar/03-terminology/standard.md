...
## لماذا يستثنى اسم الحرف وحده

اسم الحرف لا يحمل معنى وراء صوته، فإذا اشتققناه ضاع الشيء الوحيد الذي فيه.
وبقية المصطلحات كلمات لها معنى، والاشتقاق يحفظه.

وهذا الحد مقيس لا مختار. فأسماء الحروف يغلب فيها المد في الاستعمال، وبقية
المصطلحات يغلب فيها القصر:
...
## المصطلح الوحيد الذي يخالف قياسه

`tajweed` أكثر استعمالًا من `tajwid` بنحو الضعف، ومع ذلك نكتب `tajwid`.

القاعدة الواحدة المطردة أنفع من استثناء يفتح لكل كلمة بابًا، فالاستثناء الواحد
يجر غيره حتى يصير المعيار ترقيعًا.

ولا يضيع الاستعمال الغالب، لأن لكل مفهوم حقلين لا حقلًا واحدًا:
...
وتسجل `tajweed` كذلك في `alternative_spellings`، فيجدها البحث والمرادفات.
...
والهمزة والعين في وسط الكلمة تحذفان كما سبق، وتبقى حركتهما: `muallim`، `qiraah`.
...
ولا يقبل فيها سطر إلا ومعه قياس يثبت أن الصورة المشتقة لا تكاد تستعمل. والذوق
ليس قياسًا، ورجحان الاستعمال وحده لا يكفي.

والفرق بين الحالتين فرق في المقدار:

```text
juz     1384  ×  juzu       1     يسجل، فالمشتق غير مستعمل
tajwid 25280  ×  tajweed 45440    لا يسجل، فالمشتق مستعمل كثيرًا
```
