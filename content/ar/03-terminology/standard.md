...
## التهجئة مشتقة لا مختارة

قواعد الأقسام من 4 إلى 8 كلها قواعد آلية، فلا تترك الـCanonical Code Spelling
للاجتهاد: تشتق من الاسم العربي المشكول بدالة واحدة.
...
- المدخل يجب أن يكون **مشكولًا**؛ فالحركات القصيرة لا تستنتج من النص المجرد،
  واستنتاجها هو عين الاجتهاد الذي نريد رفعه. وألف الوصل غير المشكولة ترفض
...
كنا نطلب من الاسم الواحد أن يؤدي أربع وظائف مختلفة: أن يكون معرفًا ثابتًا في
الكود، واسمًا مألوفًا للقارئ، ونقلًا أمينًا للنطق، ومدخلًا للبحث. وهذه الوظائف
مطالبها متعارضة، فيقع الخلاف في `nun` أو `noon` ولا ينتهي.

والحل أن يفرد لكل وظيفة حقلها، فلا يزاحم أحدها الآخر:
...
- **`code`** معرف يستعمل في الكود والـAPI وقواعد البيانات. يشتق بقواعد الأقسام
  4 إلى 8 **بلا استثناء**، فمطلبه الثبات لا الألفة.
- **`display`** ما يقرؤه الناس في التوثيق والواجهات. يؤخذ من الصورة **الغالبة**
  في الكتابة الإنجليزية، ويوثق في `display_evidence` ما أثبت غلبتها؛ فهو اختيار
  بقياس لا بذوق.
- **`transliteration`** النقل الدقيق (ALA-LC أو DIN 31635) لمن احتاجه، وهو اختياري.
- **`arabic.vocalized`** الاسم العربي مشكولًا، وهو أصل الاشتقاق.
- **`alternative_spellings`** كل صورة مكتوبة معروفة، يولد منها فهرس مرادفات
  تحل به أي صورة إلى مفهومها في البحث وفي مدخلات الأدوات.

## لماذا يختلف `code` عن `display`؟

هذا مقصود لا سهو. فالقياس يثبت أن `Noon Sakinah` أغلب في الكتابة الإنجليزية
من `Nun Sakinah` بنحو أربعة أضعاف، وأن `Meem Sakinah` أغلب من `Mim Sakinah`
بأكثر من خمسة. فالألفة حقيقة يجب أن تخدم، لكن موضعها حيث يقرأ الناس، لا في
عمود قاعدة بيانات.

ولو أدخلنا الصورة المألوفة في `code` لبطلت قاعدة الاشتقاق: لأن التضعيف في
الاستعمال الجاري **معجمي لا قياسي**، يضعف في المألوف ويترك في غيره. وفي ملف
واحد ترى `sagheer` و`haqiqi` وفيهما ياء مدية واحدة، و`sughra` و`sagheer` من
أصل واحد. فما لا ينضبط بقاعدة لا يصلح أن يكون قاعدة.

فيبقى `code` مشتقًا، وتنال الألفة حقها في `display` وفي فهرس المرادفات.
