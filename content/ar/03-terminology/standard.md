...
description: اسم Canonical واحد لكل مفهوم، وتهجئة برمجية تُشتق بقاعدة بدل أن تُختار.
status: draft
sidebar:
  order: 1
---
...
لكل مفهوم اسم Canonical واحد، وهو الاسم الذي يُستخدم افتراضيًا في الكود في كل المشاريع التي تتبع المعيار.
...
يمكن تسجيل أسماء وتهجئات وترجمات أخرى، لكنها لا تنافس الاسم Canonical.
...
> **مفتوح:** `ت` و`ط` كلاهما يعطي `taa`، و`ح` و`ه` كلاهما يعطي `haa`، لأن §4 يجمع المفخم والمرقق. ولا يستعمل أي مدخل اليوم أحدهما اسمًا. والمسألة مسجلة مفتوحة في سجل القرارات.
...
يجب أن تبقى Canonical Code Spelling ثابتة، بينما يمكن أن تختلف طريقة العرض بحسب اللغة والجمهور والسياق.
...
وهذه القيم الست من نوع `classification_value`، لكنها مرسومة في المصحف، لذلك تحمل `symbol` و`unicode` و`mark_family` كما تحملها العلامة. ولا تحمل هذه الحقول قيمة أخرى، ويرفضها `tools/check_conformance.py` في أي مدخل آخر ليس علامة.
...
وهذه الستة مداخل لأن لكل واحد منها تعريفًا، ومعرّفاتها `ayah_numbering_kufi` و`ayah_numbering_makki` وهكذا، لأن `makki` وحدها معرّف قيمة النزول أصلًا؛ ويبين §14 لماذا يختلف الاسم البرمجي عن المعرّف. وما يخزنه البرنامج هو الاسم البرمجي.
...
ولا نكرر اسم الأب إذا لم يضف تمييزًا، فقيم `revelation_classification` تبقى `makki` و`madani` وليست `revelation_classification_makki`. واسم القيمة فريد داخل تصنيفها لا في القاموس كله، لأن العمود يحمل قيم تصنيف واحد ولا يحمل قيم اثنين. فقيم `ayah_numbering_system` هي `kufi` و`basri` و`dimashqi` و`makki` و`madani_first` و`madani_last`، و`makki` قيمة أيضًا من قيم `revelation_classification`: ولا يمد شيء يده إلى الاثنتين معًا، فلا يضطر شيء إلى الاختيار. وحيث تشترك قيمتان في اسم يكون *معرّف* المدخل، أي ملفه ومرساه وما يشير إليه `related` و`parent`، هو اسم الأب مع الاسم البرمجي: `ayah_numbering_makki`، وتحمله مداخل العد الستة كلها اطرادًا. والاسم البرمجي هو ما يخزنه البرنامج.

وهذه الستة هي العائلة الوحيدة التي لا يشتق اسمها البرمجي من العربية: فاشتقاق «العَدّ» يعطي `add` وهو فعل إنجليزي، فالاسم البرمجي اسم المدرسة. والسبب في سجل القرارات.
...
والهدف منع استخدام اسم واحد لمفهومين مختلفين في البيانات والكود، وليس توثيق الفروق اللغوية فقط.

---

## 18. الفصل بين المفاهيم المتشابهة

لا ندمج مفهومين لمجرد أن ترجمتيهما متشابهتان.
...
فالسجدة مفهومان لا واحد: العلامة المرسومة في المصحف، والموضع الذي يسجد عنده. والموضع والسجود عنده مفهوم واحد، لأن لا برنامج يخزن الفعل بمعزل عن موضعه، و«السجدات الخمس عشرة» تسمي الاثنين معًا؛ ويبين سجل القرارات لماذا فرقت مسودة سابقة بينهما.
...
`id` معرف داخلي للسجل في قاعدة البيانات:
...
`number` هو الرقم المعتمد داخل مجاله:
...
`position` هو موضع العنصر داخل أبيه أو داخل تسلسل:
...
`order` ترتيب دلالي مستقل عن الموضع:
...
ولا معنى للمفتاح إلا مع `ayah_numbering_system` مذكور. فيذكر `dataset` الذي يستخدم المفاتيح أي نظام عد يتبع، ويكون النظام `kufi` إذا لم يذكر. أما الفهرس العام للآيات (من 1 إلى 6236) فهو `position` وليس مفتاحًا، وهو كذلك يتبع نظام عد.
...
ويعرِّف `recitation_id` التسجيل. والتلاوة هي قارئ واحد يقرأ برواية واحدة بنمط واحد، ونسجل هذه الثلاثة حقولًا في التلاوة، ولا ندخلها في اسم المدى الصوتي.
...
### الاسم الذي لم تختره يُنقل ولا يُتبنى
...
المنشور. فالاسم المنقول لا يصير اسما لمفهوم، والاسم الذي تختاره لا يبقى على تهجئة
المورد لأنه دخل مع البيانات.

ويعرَّف المدقق بالأسماء المنقولة ليقرأ ما بعدها بدل أن يطلب تغييرا يقطع الإحالة:
...
وصورة `kebab` طريقة لعرض الاسم البرمجي في URL وليست تهجئة، لذلك لا نسجلها في `alternative_spellings`.
...
- **نذكر المصطلح باسمه المعتمد**، أي بقيمة `display` في مدخله: فنكتب `Ayah` وليس `verse`، و`Mushaf` وليس `codex`. والمقابل الإنجليزي في `english_glosses` قيمة للبحث، ولا نستعمله اسمًا في النص.
- **نكتب الاسم البرمجي كما هو** بين علامتي `` ` ``، ولا نترجمه ولا نعرّبه.
- **نترجم الصيغة المكررة بصيغة مكررة**: فمداخل عد الآي الستة تشترك في جملة غرض واحدة بالعربية، لذلك تشترك في جملة واحدة بالإنجليزية بلفظها.
